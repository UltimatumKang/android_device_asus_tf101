...
# device
$(call inherit-product-if-exists, device/asus/tf101/full_tf101.mk)
...
PRODUCT_BRAND := Asus
PRODUCT_NAME := ukg_tf101
PRODUCT_MODEL := Transformer
PRODUCT_MANUFACTURER := Asus
PRODUCT_PROPERTY_OVERRIDES += ro.buildzipid=ukg.tf101.$(shell date +%m%d%y).$(shell date +%H%M%S)
...
PRODUCT_BUILD_PROP_OVERRIDES += \
    ...
    PRIVATE_BUILD_DESC="tf101-user 4.2.2 JDQ39 573038 release-keys" \
    BUILD_FINGERPRINT="asus/tf101:4.2.2/JDQ39/573038:user/release-keys"

# hybrid
PRODUCT_COPY_FILES +=  \
    vendor/ukg/prebuilt/hybrid_mdpi.conf:system/etc/beerbong/properties.conf
